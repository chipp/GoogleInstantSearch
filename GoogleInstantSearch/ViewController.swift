...
class ViewController: UIViewController {
  ...
  @IBOutlet private weak var keywordTextField: UITextField! {
    didSet {
      ...
      let padding = UIView(frame: CGRect(origin: CGPoint.zero,
        size: CGSize(width: 8, height: keywordTextField.frame.height)))
  ...
  override func viewDidLoad() {
    ...
    activityIndicator.drive(
      UIApplication.sharedApplication().rx_networkActivityIndicatorVisible)
      .addDisposableTo(disposeBag)

    keywordTextField.rx_text.asDriver()
      // .throttle(0.5) – will wait 0.5s for new value from textField,
      // if no value received – send latest value to subscriber
      ...
      .flatMapLatest { [weak self] keyword -> Driver<[NSAttributedString?]> in
        ...
        return Observable.zip(Observable.just(keyword),
          networkRequest) { keyword, suggestions -> [NSAttributedString?] in
      ...
      .drive(tableView.rx_itemsWithCellIdentifier("SuggestCell",
            cellType: UITableViewCell.self)) { row, suggest, cell in
